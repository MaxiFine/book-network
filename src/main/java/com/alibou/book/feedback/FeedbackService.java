...
public class FeedbackService {
    ...
    public Integer save(FeedbackRequest request, Authentication connectedUser) {
        ...
        if (!book.isArchived() || !book.isShareable()){
            throw new OperationNotPermittedException("You cannot give a feedback for an archived book or a shared book.");
        }
    ...
    public PageResponse<FeedbackResponse> findAllFeedbackByBook(Integer bookId, int page, int size, Authentication connectedUser) {
        ...
        List<FeedbackResponse> feedbackResponses = feedbacks.stream()
                .map(feeds -> feedbackMapper.toFeedbackResponse(feeds, user.getId()))
                .toList();
